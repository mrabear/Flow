...
var Balls = 
{
	...
	TranslateBallPositions: function(XOffset, YOffset)
	{
		...
		for( var CurrentBall = 0 ; CurrentBall < BallTotal ; CurrentBall++ )
		{
			...
			if( Balls.Instances[CurrentBall].x > Graphics.CenterPoint.x )
					Balls.Instances[CurrentBall].x += XOffset;

			// If the ball is past the center Y point, translate Y position by YOffset
			if( Balls.Instances[CurrentBall].y > Graphics.CenterPoint.y )
					Balls.Instances[CurrentBall].y += YOffset;
		}
	}
};